...
class ExtremumStrategyV6(StrategyTemplate):
    """
    极大极小值策略 v6 - 增强版极值识别策略
    ...
    def __init__(self, name="极大极小值策略v6", data=None, parameters=None):
        default_params = {
            # 极值识别基础参数
    ...
    def calculate_position_size(self, signal_strength, current_position):
        """根据信号强度和当前仓位计算建议仓位大小"""
        base_size = self.parameters.get("base_position_size", 0.2)
                    ...
                    for extremum_idx in minima_candidates:
                        if extremum_idx not in processed_extremums:
                            ...
                            if strength >= self.parameters.get("signal_strength_threshold", 0.65):
                                ...
                                position_size = self.calculate_position_size(strength, current_position)
                                trigger_reason = f"极小值买入: 强度{strength:.2f}, 极值位置{extremum_idx}"
                    ...
                    for extremum_idx in maxima_candidates:
                        if extremum_idx not in processed_extremums:
                            ...
                            if strength >= self.parameters.get("signal_strength_threshold", 0.65):
                                ...
                                signal_strength = strength
                                position_size = current_position
                                trigger_reason = f"极大值卖出: 强度{strength:.2f}, 极值位置{extremum_idx}"
                                processed_extremums.add(extremum_idx)
                                break
